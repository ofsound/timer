...
function Map({ sequenceArray, thisStep, thisRatio, isHistoryMap, onClick }: inputProps) {
  return (
    <div
      ...
      className={`${isHistoryMap ? "mt-4 flex min-w-0 gap-1 border-none" : "mt-6 flex min-h-15 max-w-full min-w-0 flex-1 justify-start gap-2 rounded-lg border border-dashed border-gray-300 p-2"}`}
    >
