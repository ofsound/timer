...
function Map({ sequenceArray, thisStep, thisRatio, isHistoryMap, onClick }: inputProps) {
  return (
    <div
      ...
      className={`${thisStep === 0 && !isHistoryMap && "animate-pulse-fast"} ${isHistoryMap ? "mt-3.5 flex min-w-0 gap-1 border-none" : "flex h-full w-full max-w-full min-w-0 justify-start gap-2 rounded-lg border border-dashed border-gray-300 p-2"}`}
    >
