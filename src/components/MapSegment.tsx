...
function MapSegment({ isActive, isComplete, durationSeconds, progressRatio, isHistoryMapSegment }: inputProps) {
  ...
  return (
    <div
      ...
      className={`${isComplete && "border-gray-800 text-gray-800 !opacity-15"} ${isActive && ""} ${isHistoryMapSegment ? "relative h-7 overflow-hidden rounded-lg border border-black bg-gray-400 text-center text-black first:hidden even:bg-gray-100" : "relative h-full overflow-hidden rounded-lg border border-black bg-gray-400 text-center text-black first:hidden even:bg-gray-100"} `}
    >
      <div
