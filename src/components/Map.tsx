...
function Map({ isHistoryMap, onClick, historySequence }: inputProps) {
  ...
  return (
    <div
      ...
      style={styles}
      className={`${thisStep === 0 && !isHistoryMap && ""} ${isHistoryMap ? "mt-4 flex min-w-0 gap-1 border-none" : "flex h-full max-h-20 w-full max-w-full min-w-0 justify-start gap-2 rounded-lg border border-dashed border-gray-300 p-2"}`}
    >
      {!isHistoryMap &&
